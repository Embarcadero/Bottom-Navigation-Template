...
type
  ...
  function ShowBasicForm(): TfrmBasic;
...
function TfrmBasic.RunDialog(const SuccProc: TProc): string;
begin
  FSuccProc:= SuccProc;
...
begin
  ...
  {if FFormStyle = fmLight then
  begin
    ColorAnimation1.StopValue:= TAlphaColor($ff1c6ecd);
    ColorAnimation2.StopValue:= TAlphaColor($ff1c6ecd);
    ColorAnimation3.StopValue:= TAlphaColor($ff1c6ecd);

    btnRecents.StylesData['btnTextColorAnimation.StopValue']:= TValue.From<TAlphaColor>(TAlphaColor($ff1c6ecd));
    btnFavorites.StylesData['btnTextColorAnimation.StopValue']:= TValue.From<TAlphaColor>(TAlphaColor($ff1c6ecd));
    btnNearby.StylesData['btnTextColorAnimation.StopValue']:= TValue.From<TAlphaColor>(TAlphaColor($ff1c6ecd));

    PanelBottom.StyleLookup:= 'PanelBottomLightStyle';
  end;}
...
procedure TfrmBasic.btnBottomMenuClick(Sender: TObject);
begin
...
procedure TfrmBasic.tFormShowedTimer(Sender: TObject);
begin
  if Width > 0 then
  begin
...
procedure TfrmBasic.InsertTestDataToTheMemTables();
var
...
procedure TfrmBasic.LoadTopListboxItems(aListBox: TListBox);
var
...
procedure TfrmBasic.ListBoxTopItemClick(Sender: TObject);
begin
...
procedure TfrmBasic.ListBoxItemButtonClick(Sender: TObject);
begin
  if TListBoxItem(Sender).Tag > 0 then
  begin
...
procedure TfrmBasic.Load3PartsListboxItems(aListBox: TListBox);
var
  lLBItem: TListBoxItem;
  lMemoryStream: TMemoryStream;
begin
...
procedure TfrmBasic.ListBox3PartsItemClick(Sender: TObject);
begin
  if TListBoxItem(Sender).Tag > 0 then
  begin
...
procedure TfrmBasic.LoadBottomListboxItems(aListBox: TListBox);
var
  lLBItem: TListBoxItem;
  lMemoryStream: TMemoryStream;
begin
...
procedure TfrmBasic.ListBoxBottomItemClick(Sender: TObject);
begin
  if TListBoxItem(Sender).Tag > 0 then
  begin
...
procedure TfrmBasic.VertScrollBoxMainViewportPositionChange(Sender: TObject; const OldViewportPosition, NewViewportPosition: TPointF; const ContentSizeChanged: Boolean);
...
procedure TfrmBasic.eSearchEnter(Sender: TObject);
begin
...
procedure TfrmBasic.eSearchExit(Sender: TObject);
begin
